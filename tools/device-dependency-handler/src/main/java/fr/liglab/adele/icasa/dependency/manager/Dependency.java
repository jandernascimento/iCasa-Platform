...
import fr.liglab.adele.icasa.dependency.manager.ServiceUsage.Usage;
import fr.liglab.adele.icasa.dependency.manager.util.DependencyModel;
...
public class Dependency extends DependencyModel implements FieldInterceptor, MethodInterceptor, ConstructorInjector {
	...
	/**
	 * List of dependency callback. Immutable once set.
	 */
	private DependencyCallback[] m_callbacks;

	/**
	 * Is the dependency a service level dependency. Immutable once set.
	 */
	...
	/**
	 * Type of the object to inject. Cannot change once set.
	 */
	private int m_type;

	/**
	 * Nullable object. Immutable once set.
	 */
	...
	/**
	 * Id of the dependency. Immutable once set.
	 */
	private String m_id;
	...
	/**
	 * Constructor paramter index. -1 if not used.
	 */
	...
	private final List m_pendingRefs = new ArrayList();
	 ...
	 * 
	 * @param handler
	 *           : the dependency handler managing this dependency
	 * @param field
	 *           : field of the dependency
	 * @param spec
	 ...
	 * @param isAggregate
	 *           : is the dependency an aggregate dependency
	 * @param nullable
	 *           : describe if the nullable ability is enable or disable
	 * @param isProxy
	 *           : is the proxied dependency
	 * @param identity
	 *           : id of the dependency, may be null
	 * @param context
	 *           : bundle context (or service context) to use.
	 * @param policy
	 *           : resolution policy
	 * @param cmp
	 *           : comparator to sort references
	 * @param defaultImplem
	...
	public Dependency(DependencyHandler handler, String field, Class spec, Filter filter, boolean isOptional,
	      ...
	      Comparator cmp, String defaultImplem, String mandatoryProps) {
		...
		m_isProxy = isProxy;
		m_context = context;
		m_mandatoryProps = mandatoryProps;
	...
	/**
	 * Set the specification of the current dependency. In order to store the id
	 * of the dependency, this method is override. This method is called during
	 * the configuration.
	 * 
	 * @param spec
	...
	/**
	 * Add a callback to the dependency. This method is called during the
	 * configuration.
	 * 
	 * @param callback
	 *           : callback to add
	 ...
	 * 
	 * @param pojo
	 *           : pojo instance on which calling the bind method.
	 */
	protected void onObjectCreation(Object pojo) {
		...
		synchronized (this) {
			...
			// We are notified of an instance creation, we have to freeze when the
			// static policy is used
		...
		// Call bind callback.
		for (int j = 0; m_callbacks != null && j < m_callbacks.length; j++) { // The
																									 // array
																									 // is
																									 // constant.
	 ...
	 * 
	 * @param ref
	 *           : reference to send (if accepted) to the method
	 */
	 ...
	 *           : service reference.
	 * @param pojo
	 *           : pojo on which calling the callback, if null call on each
	...
	private void invokeCallback(DependencyCallback callback, ServiceReference ref, Object pojo) {
		try {
			...
			m_handler.error("The method " + callback.getMethodName() + " does not exist in the implementation class "
			      + m_handler.getInstanceManager().getClassName(), e);
			m_handler.getInstanceManager().stop();
		} catch (IllegalAccessException e) {
			m_handler.error("The method " + callback.getMethodName() + " is not accessible in the implementation class "
			      + m_handler.getInstanceManager().getClassName(), e);
			m_handler.getInstanceManager().stop();
		} catch (InvocationTargetException e) {
			m_handler.error("The method " + callback.getMethodName() + " in the implementation class "
	...
	/**
	 * Call 'modify' method with the service reference in parameter (if
	 * accepted).
	 * 
	 * @param ref
	...
	private Object createNullableObject() {
		...
		try {
			ClassLoader cl = new NullableClassLoader(getHandler().getInstanceManager().getClazz().getClassLoader(),
			      getSpecification().getClassLoader());

			m_nullable = Proxy.newProxyInstance(cl, new Class[] { getSpecification(), Nullable.class },
			      new NullableObject()); // NOPMD

		} catch (NoClassDefFoundError e) {
			// A NoClassDefFoundError is thrown if the specification uses a class
			// not accessible by the actual instance.
			// It generally comes from a missing import.
			throw new IllegalStateException("Cannot create the Nullable object, a referenced class cannot be loaded: "
			      + e.getMessage());
		} catch (Throwable e) { // Catch any other exception that can occurs
			throw new IllegalStateException("Cannot create the Nullable object, an unexpected error occurs: "
			      + e.getMessage());
		}
		...
		if (isOptional() && !isAggregate()) {
			if (m_di == null) {
				...
				try {
					...
					throw new IllegalStateException("Cannot load the default-implementation " + m_di + " : "
					      + e.getMessage());
					...
					throw new IllegalStateException("Cannot load the default-implementation (unexpected exception) " + m_di
					      + " : " + e.getMessage());
				}
			}
		}
		...
		if (m_isProxy) {
			if (isAggregate()) {
				...
				if (getSpecification().isInterface()) {
					String type = getHandler().getInstanceManager().getContext()
					      .getProperty(DependencyHandler.PROXY_TYPE_PROPERTY);
	...
	/**
	 * Set that this dependency is a service level dependency. This forces the
	 * scoping policy to be STRICT.
	 */
	public void setServiceLevelDependency() {
		m_isServiceLevelRequirement = true;
		setBundleContext(new PolicyServiceContext(m_handler.getInstanceManager().getGlobalContext(), m_handler
		      .getInstanceManager().getLocalServiceContext(), PolicyServiceContext.LOCAL));
	}
	 ...
	 * 
	 * @param reference
	 *           : the new matching service reference.
	 * @see org.apache.felix.ipojo.util.DependencyModel#onServiceArrival(org.osgi.framework.ServiceReference)
	 */
	public void onServiceArrival(ServiceReference reference) {
		callBindMethod(reference);
		// The method is only called when a new service arrives, or when the used
		// one is replaced.
	}
	...
	/**
	 * The dependency has been reconfigured. Call unbind method and then bind
	 * methods. If the dependency cache is not reset, the thread continues to get
	 * older services.
	 * 
	 * @param departs
	 *           : no more matching services.
	 * @param arrivals
	 *           : new services
	 ...
	 * 
	 * @return the used service reference or null if no service reference are
	 *         available.
	 */
	...
	/**
	 * Called by the proxy to get service objects to delegate a method. On
	 * aggregate dependencies, it returns a list.
	 * 
	 * @return a service object or a nullable/default-implementation object. For
	 *         aggregate dependencies it returns a list or an empty list.
	 */
	...
	/**
	 * This method is called by the replaced code in the component implementation
	 * class. Construct the service object list is necessary.
	 * 
	 * @param pojo
	 *           : POJO object.
	 * @param fieldName
	 ...
	 * @param value
	 *           : last value.
	 * @return the service object or a nullable / default implementation if
	 *         defined.
	...
	/**
	 * Creates the object to store in the given Thread Local. This object will be
	 * injected inside the POJO field.
	 * 
	 * @param usage
	...
	private void createServiceObject(Usage usage) {
		...
		if (!isAggregate()) {
			if (refs == null) {
				...
				usage.m_object = m_nullable; // Add null if the Nullable pattern is
													  // disable.
			} else {
				ServiceReference ref = getServiceReference();
			...
			if (m_type == 0) { // Array
				try {
					if (refs == null) {
						usage.m_object = (Object[]) Array.newInstance(getSpecification(), 0); // Create
																													 // an
																													 // empty
																													 // array.
					} else {
						// Use a reflective construction to avoid class cast
						// exception. This method allows setting the component type.
					...
					throw new RuntimeException(
					      "Cannot create the array - Check that the bundle can access the service interface : "
					            + e.getMessage());
				}
	...
	/**
	 * The field was set. This method should not be call if the POJO is written
	 * correctly.
	 * 
	 * @param pojo
	 *           : POJO object
	 ...
	 * @param args
	 *           : arguments
	 * @see org.apache.felix.ipojo.MethodInterceptor#onEntry(java.lang.Object,
	...
	/**
	 * A POJO method has thrown an error. This method does nothing and wait for
	 * the finally.
	 * 
	 * @param pojo
	 *           : POJO object.
	 ...
	 * @param throwable
	 *           : thrown error
	 * @see org.apache.felix.ipojo.MethodInterceptor#onError(java.lang.Object,
	 ...
	 * @see org.apache.felix.ipojo.MethodInterceptor#onExit(java.lang.Object,
	 *      java.lang.reflect.Method, java.lang.Object)
	 */
	 ...
	 * @param method
	 *           : Method object.
	 * @see org.apache.felix.ipojo.MethodInterceptor#onFinally(java.lang.Object,
	...
	private class NullableClassLoader extends ClassLoader {
		 ...
		 * @param spec
		 *           the specification class loader.
		 */
		...
		/**
		 * Loads the given class. This method uses the classloader of the
		 * component class and (if not found) the specification classloader.
		 * 
		 * @param name
		 *           the class name
		 * @return the class object
		 * @throws ClassNotFoundException
		 *            if the class is not found by the two classloaders.
	...
	private class SmartProxyFactory extends ClassLoader {
		 ...
		 * 
		 * @param clazz
		 *           the service specification to proxy
		...
		/**
		 * Create a proxy object for the given specification. The proxy uses the
		 * given dependency to get the service object.
		 * 
		 * @param spec
		...
		public Object getProxy(Class spec, Dependency dep) {
			try {
				...
				Constructor constructor = clazz.getConstructor(new Class[] { clazz.getClassLoader().loadClass(
		...
		/**
		 * Loads the given class. This method uses the classloader of the
		 * specification class or the handler class loader.
		 * 
		 * @param name
		 *           the class name
		 * @return the class object
	...
	private class DynamicProxyFactory implements InvocationHandler {
		...
		/**
		 * Creates a proxy object for the given specification. The proxy uses the
		 * given dependency to get the service object.
		 * 
		 * @param spec
		 *           the service specification (interface)
	 ...
	 * 
	 * @return the index of the constructor parameter, or <code>-1</code> if not
	 *         set.
	 */
	...
	public boolean match(ServiceReference ref) {
	   ...
	   if (service instanceof GenericDevice) {
	      ...
	      String temProps = m_mandatoryProps.substring(1, m_mandatoryProps.length()-1);
	      
	      
	      String[] deviceProps = temProps.split(",");
	      
	      for (String deviceProperty : deviceProps) {
	         System.out.println("To match ---> " + deviceProperty);
	         if (device.getPropertyValue(deviceProperty)==null) {
		         match = false;
		         
		         synchronized (m_pendingRefs) {
		         	m_pendingRefs.add(ref);		         	
               }
		         device.addListener(new HandlerDeviceListener(ref));
		         break;	         	
	         }
         }	      
      }
	   System.out.println("Service matching ---> " + match);
	   return match;
	}
