...
public class DemoActivity extends Activity implements PropertyChangeListener,SensorEventListener {
	 ...
	 */
	static float xGyroscope, yGyroscope, zGyroscope;
	/**
	...
	private UDN udn = UDN.uniqueSystemIdentifier("Demo Gyroscope");
	...
	private ServiceConnection serviceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			LocalService<Gyroscope> gyroscopeService = getGyroscopeService();
			...
			if (gyroscopeService == null) {
				try {
					...
					gyroscopeService = getGyroscopeService();
			...
			gyroscopeService.getManager().getImplementation().getPropertyChangeSupport()
	...
	protected void onDestroy() {
		...
		LocalService<Gyroscope> switchPowerService = getGyroscopeService();
	...
	public void propertyChange(PropertyChangeEvent event) {
		if (event.getPropertyName().equals("xGyroValue")) {
	...
	protected LocalService<Gyroscope> getGyroscopeService() {
		if (upnpService == null)
			return null;
		...
		return (LocalService<Gyroscope>)
				binaryLightDevice.findService(new UDAServiceType("Gyroscope", 1));
	}
	...
	protected LocalDevice createDevice()
		...
		DeviceType type =
				new UDADeviceType("Gyroscope", 1);

		DeviceDetails details =
				new DeviceDetails(
						"Friendly Gyroscope",
						new ManufacturerDetails("ADELE"),
						new ModelDetails("AndroidGyro", "A demo of gyroscope sensor.", "v1")
						);

		LocalService service =
				new AnnotationLocalServiceBinder().read(Gyroscope.class);

		service.setManager(
				new DefaultServiceManager<Gyroscope>(service, Gyroscope.class)
				);
