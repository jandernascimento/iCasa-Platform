...
public class DemoActivity extends Activity implements PropertyChangeListener,SensorEventListener {
	...
	private UDN udn = UDN.uniqueSystemIdentifier("Demo Photometer");
	...
	private ServiceConnection serviceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			LocalService<Photometer> photometerService = getPhotometerService();

			// Register the device when this activity binds to the service for the first time
			if (photometerService == null) {
				try {
					...
					photometerService = getPhotometerService();
			...
			photometerService.getManager().getImplementation().getPropertyChangeSupport()
	...
	protected void onDestroy() {
		...
		LocalService<Photometer> switchPowerService = getPhotometerService();
	...
	public void propertyChange(PropertyChangeEvent event) {
		if (event.getPropertyName().equals("Illuminance")) {
			log.info("Turning light: " + event.getNewValue());
			//			l = ;
			//			setLightbulb((Float) event.getNewValue());
			lightValue = (TextView) findViewById(R.id.txtView);
		}
	}

	protected LocalService<Photometer> getPhotometerService() {
		if (upnpService == null)
			return null;
		...
		return (LocalService<Photometer>)
				binaryLightDevice.findService(new UDAServiceType("Photometer", 1));
	}
	...
	protected LocalDevice createDevice()
		...
		DeviceType type =
				new UDADeviceType("Photometer", 1);

		DeviceDetails details =
				new DeviceDetails(
						"Friendly Photometer",
						new ManufacturerDetails("ADELE"),
						new ModelDetails("AndroidLight", "A demo of photometer sensor.", "v1")
						);

		LocalService service =
				new AnnotationLocalServiceBinder().read(Photometer.class);

		service.setManager(
				new DefaultServiceManager<Photometer>(service, Photometer.class)
				);
	...
	protected Icon createDefaultDeviceIcon() {
		return new Icon(
				...
				"89504E470D0A1A0A0000000D494844520000002D000000300803000000B28C03ED0000001974455874536F6674" +
						...
						"04408001003EE42959E2CD74A60000000049454E44AE426081"
				);
	}
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (event.sensor.getType() == Sensor.TYPE_LIGHT) {
			...
			if(photometerService != null){
				...
				l= event.values[0];
				if(oldIllumianceValue != l && ((l>(oldIllumianceValue+5) || (l<(oldIllumianceValue-5))))){
					photometerService.getManager().getImplementation().getPropertyChangeSupport()
					.firePropertyChange("Illuminance", oldIllumianceValue, l);
				}
